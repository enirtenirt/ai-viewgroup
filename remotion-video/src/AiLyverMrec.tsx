...
const ViewLogoCorner: React.FC<{ opacity?: number; dark?: boolean }> = ({ opacity = 1, dark = false }) => (
  <div style={{ position: "absolute", top: 10, right: 12, opacity }}>
    <Img
...
const S1: React.FC = () => {
  ...
  return (
    <AbsoluteFill
      style={{
        background: `radial-gradient(ellipse at 30% 30%, ${C.primary} 0%, ${C.dark} 65%, #0a0210 100%)`,
        fontFamily: FONT,
        color: "#fff",
    ...
    >
      ...
      >
        {words.map((w, i) => {
          ...
          const y = interpolate(frame, [d, d + 16], [10, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
          return (
            <span
              key={i}
              style={{
...
const S3: React.FC = () => {
  ...
  return (
    <AbsoluteFill
      style={{
        background: `radial-gradient(ellipse at 70% 60%, ${C.primary} 0%, ${C.dark} 65%, #0a0210 100%)`,
        fontFamily: FONT,
        color: "#fff",
    ...
    >
      ...
      >
        ...
        <span style={{ textDecoration: "underline", textDecorationColor: C.pop, textUnderlineOffset: 3 }}>
          datagrunnlaget
        </span>{" "}
        – ikke modellene.
      </h2>
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              boxShadow: row.accent ? `0 0 18px ${C.pop}66` : "none",
              fontSize: 10,
              letterSpacing: 1.5,
              fontWeight: 700,
          ...
          >
            <span>{row.label}</span>
            {row.accent && <span style={{ fontSize: 9 }}>← START</span>}
          </div>
        ))}
      </div>
    </AbsoluteFill>
...
const S4: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 10, letterSpacing: 2.5, fontWeight: 700, color: C.primary, opacity: logoOp }}>
        ÉN SANNHET
      </div>
      <h2
        style={{
...
export const AiLyverMrec: React.FC = () => {
  return (
